...
{
	return ext ? translate_eth_ext_proto_oper(eth_proto_oper, active_speed,
											  active_width)
							 ...
							 struct mlx5_ib_alloc_ucontext_resp *resp)
{
	...
	resp->log_uar_size = MLX5_CAP_GEN(dev->mdev, uar_4k) ? MLX5_ADAPTER_PAGE_SHIFT : PAGE_SHIFT;
	resp->num_uars_per_page = MLX5_CAP_GEN(dev->mdev, uar_4k) ? MLX5_CAP_GEN(dev->mdev,
																			 num_of_uars_per_page)
															  : 1;
	resp->tot_bfregs = bfregi->lib_uar_dyn ? 0 : bfregi->total_num_bfregs - bfregi->num_dyn_bfregs;
					...
					struct mlx5_ib_ucontext *context)
{
	...
	int max_valid_idx = dyn_uar ? bfregi->num_sys_pages : bfregi->num_static_sys_pages;
							  ...
							  struct ib_event *ibev)
{
	...
	case MLX5_PORT_CHANGE_SUBTYPE_INITIALIZED:
		...
		ibev->event = (eqe->sub_type == MLX5_PORT_CHANGE_SUBTYPE_ACTIVE) ? IB_EVENT_PORT_ACTIVE : IB_EVENT_PORT_ERR;
		break;
										...
										size_t count, loff_t *pos)
{
	...
	timeout = min_t(u32, roundup(var, 100), MLX5_MAX_DELAY_DROP_TIMEOUT_MS * 1000);
	if (timeout != var)

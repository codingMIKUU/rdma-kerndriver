...
// {
//     sched->task = kthread_create(scheduler_polling,(void*)sched,"polling thread");
// 	sched->sq_head = NULL;
//     sched->cq_head = NULL;
...
//     pr_info("in is_xrc_exists,gid.in_id = %llu, gid.subnet = %llu\n",dgid->global.interface_id,dgid->global.subnet_prefix);
//     struct mlx5_ib_srmc *srmc;
//     int ret = 1;
//     mutex_lock(&sched->srmc_lock);
//     for(srmc=sched->srmc_head;srmc;srmc=srmc->next){
...
//     cq = __ib_create_cq(pd->device,NULL,NULL,NULL,&cq_attr,"scheduler");
//     if(!cq){
